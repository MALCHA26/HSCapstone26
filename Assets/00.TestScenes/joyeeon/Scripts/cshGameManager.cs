...
public class cshGameManager : MonoBehaviourPun // 점수와 게임 오버 여부 및 게임 UI를 관리하는 게임 매니저 스크립트
{
    ...
    private void Start()
    {
        ...
        if (PlayerInfo.playerInfo == 1)
            PhotonNetwork.Instantiate(MocapPlayerPrefab.name, randomSpawnPos, Quaternion.identity);
